...
@else
    <figure {{ $attributes->class('m-0 flex flex-col items-center gap-3') }}>
        <svg viewBox="0 0 100 100" style="width: {{ $size }}px; height: {{ $size }}px" class="text-eh-text" role="img"
            aria-label="{{ $description ?? 'Distribución por categoría' }}">
            <circle cx="50" cy="50" r="{{ $radio }}" fill="none" stroke="currentColor" stroke-opacity=".09" stroke-width="9" />
            @foreach ($filas as $fila)
                @php
                    ...
                    $acumulado += $porcion;
                @endphp
                <circle class="donut-seg" cx="50" cy="50" r="{{ $radio }}" fill="none" stroke="{{ $fila['color'] }}" stroke-width="9"
            ...
            @endforeach
            <text x="50" y="50" text-anchor="middle" class="mono" style="font-size: 16px; font-weight: 600; letter-spacing: -.05em; fill: var(--eh-text)">{{ number_format($total) }}</text>
            <text x="50" y="60" text-anchor="middle" style="font-size: 5px; letter-spacing: .12em; fill: var(--eh-text-muted)">TOTAL</text>
        </svg>
        <ul class="flex w-full flex-wrap items-center justify-center gap-x-4 gap-y-1.5">
            @foreach ($filas as $fila)
                <li class="flex items-center gap-1.5 text-[11px] text-eh-text-muted">
                    <span class="h-2 w-2 shrink-0 rounded-sm" style="background: {{ $fila['color'] }}"></span>
