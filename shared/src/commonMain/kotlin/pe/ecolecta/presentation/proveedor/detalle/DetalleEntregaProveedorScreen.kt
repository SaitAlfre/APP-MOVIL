...
fun DetalleEntregaProveedorScreen(
    id: String,
    viewModel: DetalleEntregaProveedorViewModel = koinViewModel(key = id, parameters = { parametersOf(id) }),
) {
    val estado by viewModel.uiState.collectAsState()

    when {
        estado.cargando -> IndicadorCarga(mensaje = "Cargando entrega…")
        estado.error != null -> EstadoVacio(titulo = "No se pudo cargar la entrega", descripcion = estado.error.orEmpty())
        estado.noEncontrada -> EstadoVacio(titulo = "No se encontró esta entrega")
            ...
            Column(
                ...
                Tarjeta {
                    Text("Estado de sincronización", color = Colores.textSecundario, style = MaterialTheme.typography.bodyMedium)
                    Row(Modifier.fillMaxWidth().padding(top = Espaciado.xs)) {
                        ChipSync(entrega, larga = true)
                    }
                }
            }
        }
